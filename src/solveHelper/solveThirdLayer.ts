...
export function solveThirdLayer(cube: Cube) {
	...
	let helper = {
		...
		positionFirstCorner: (): void => {
			if(helper.getThirdLayer(Faces.F)[0] === 'F' && helper.getThirdLayer(Faces.L)[2] === 'L') {
				while(!thirdLayerSolved(cube)) { console.log(1); cube.move('R F\' R B B R\' F R B B R R'); }
				return;
			}
			if(helper.getThirdLayer(Faces.F)[2] === 'F' && helper.getThirdLayer(Faces.R)[0] === 'R') {
				while(!thirdLayerSolved(cube)) {
				console.log(helper.getThirdLayer(Faces.F), helper.getThirdLayer(Faces.L), helper.getThirdLayer(Faces.B), helper.getThirdLayer(Faces.R));
				console.log(2); cube.move('B R\' B L L B\' R B L L B B'); }
				return;
			}
			if(helper.getThirdLayer(Faces.B)[2] === 'B' && helper.getThirdLayer(Faces.L)[0] === 'L') {
				while(!thirdLayerSolved(cube)) { console.log(3); cube.move('F L\' F R R F\' L F R R F F'); }
				return;
			}
			if(helper.getThirdLayer(Faces.B)[0] === 'B' && helper.getThirdLayer(Faces.R)[2] === 'R') {
				while(!thirdLayerSolved(cube)) { console.log(4); cube.move('L B\' L F F L\' B L F F L L'); }
				return;
			}
	...
	console.log(1, cube.faces[Faces.B].data[0]);
	helper.positionEdges();
	console.log(2, cube.faces[Faces.B].data[0]);
	helper.positionFirstCorner();
	console.log(3, cube.faces[Faces.B].data[0]);
